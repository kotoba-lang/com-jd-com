(ns jd_com.main
  "Kotodama WASM entrypoint for the Jd_com clean-room actor (L4) — Clojure port.
  ...
  facts (`jd_com.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jd_com")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "jdcom_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "jdcom_cus"
    :fields [:email :firstName :lastName :phone]                  :required [:email :firstName]
    :coerce {}                                                     :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "jdcom_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"jd_com.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jd_com-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jd_com-compat/ready)
